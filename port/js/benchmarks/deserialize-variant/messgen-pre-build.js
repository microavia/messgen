...
export class Struct {
  ...
  _init() {
    ...
    this._default = Object.fromEntries(this._fields.map((f) => {
      let value
      if (f._prop.isArray) {
        let TypedArray = basicTypes[f._prop.typeIndex].typedArray
        return [f.name, TypedArray(f._prop.length)]
      } else if (f._prop.isComplex) {
        return [f.name, null]
      } else {
        return [f.name, basicTypes[f._prop.typeIndex].default]
      }
    }))
    this._size = offset
  }
...
export class Buffer {
  ...
  static serializeMessage(struct, obj, headerStruct = HEADER_STRUCT, includeMessages) {
    ...
    let headerBuf = Buffer.serializeObj(
      headerStruct.fields,
      {
        seq: obj.seq,
        size: messageSize,
  ...
  __deserialize__(struct, offset, sizeOffset) {
    ...
    let fields = struct.fields,
      dv = this._dataView,
      res = Object.create(struct._default)
